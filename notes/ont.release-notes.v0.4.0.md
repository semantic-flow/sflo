...
- **Observed constraints target property subjects.** Digest grammar, required observed spec, timestamp, and same-method checks now fire for every subject of `observedContentDigest`, even when the subject lacks an explicit `ArtifactResolutionObservation` type and no RDFS inference is enabled.
