...
* `stylesheetUrl` as a **DatatypeProperty** (xsd:string path).
...
* `defaultPlatformKnopConfig` as **instance data embedded in the ontology document**.
...
2. **Instance data in ontology**
   ...
   * `defaultPlatformKnopConfig` is ABox data living in a TBox file.
...
* `schema:domainIncludes`: `AbstractNodeConfig/`, `PlatformKnopConfig/`, `ApplicationKnopConfig/`.
...
* `hasResourcePageTemplate/` (DatatypeProperty) and `stylesheetUrl/` are legacy.
...
* Remove `defaultPlatformKnopConfig` instance from the ontology file.
* Create a new **single-mesh repo** (e.g., `sflo-platform-defaults`).
* In that repo:

  * A **platform config knop** (of class `PlatformKnopConfig/`) with an IRI, e.g.:
...
* Extraction of `defaultPlatformKnopConfig` from ontology into `sflo-platform-defaults` mesh.
...
* `defaultResourcePageTemplate/`:
  ...
  * `@type`: `owl:ObjectProperty`
  * domain: `AbstractNodeConfig/`, `PlatformKnopConfig/`, `ApplicationKnopConfig/`
...
These will eventually replace `hasResourcePageTemplate/` and `stylesheetUrl/` for new configs.
...
* `TemplatePathShape/` and any `stylesheetUrl` regex enforcement become **legacy-only**.
...
#### T1.5 Remove `defaultPlatformKnopConfig` ABox from ontology file

* Delete the `defaultPlatformKnopConfig` individual and its embedded `defaultInheritableNodeConfig` from the ontology JSON-LD.
...
#### T2.3 Define `PlatformKnopConfig/` instance using new properties

Create a `PlatformKnopConfig/` instance in the mesh (not in the ontology):

```jsonld
{
  "@id": "https://semantic-flow.github.io/platform/config/default",
  "@type": "PlatformKnopConfig/",
...
This replaces the now-removed `defaultPlatformKnopConfig` in the ontology file.
...
* Define strict precedence:
  ...
  * `hasResourcePageTemplate` / `stylesheetUrl` (strings).
