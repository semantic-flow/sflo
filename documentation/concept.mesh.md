...
- **Addressable**: Every [[NonArtifact]] has an [[concept.identifier]]; when a mesh is [[published|concept.publication]], every [[NonArtifact]] then gets a globally unique, human-readable IRI
...
[[Mesh resources|NonArtifact]] have at least one [[concept.identifier]] and (usually) a [[concept.referent]].
...
| `ns/djradon/index.html`                                              | [[ResourcePage]] | resource page (content)      |
...
Terminal [[mesh resources|NonArtifact]] that cannot contain other resources:

- **[[Resource pages|ResourcePage]]**: index.html files present in every mesh folder after weaving
...
- all resource folders should contain a [[ResourcePage]] that makes there IRIs servable/dereferenca
- 
...
- IRIs must return meaningful content when dereferenced
  - [[ResourcePage]] provide human-readable information for [[facet.filesystem.folder]]-based resources
