...
The default version provides the most recently published version of a flow's content without relying on symlinks or redirects. It serves as the "general purpose" source that users and external systems can reference, remaining unchanged during active development via the [[mesh-resource.component.slice.working-slice]].

If versioning is turned on and nobody has cleaned up old versions, the default version matches the content of the latest versioned version (e.g., `2025-11-24_0142_07_v3/`) and remains identical to the [[mesh-resource.component.slice.working-slice]] until new changes begin. During weaving, the [[mesh-resource.component.distribution.working]] becomes the new [[mesh-resource.component.slice.default-slice]]. If versioning is turned on, the _working content becomes the working version.
