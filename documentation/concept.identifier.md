...
* **bare knop identifiers** → Organizational/namespace containers
* **reference knop identifiers** → denotate things other than concrete information resources
* **payload knop identifiers** →  denote datasets in the abstract
* **flow identifiers** → denote dataset series
* **Concrete dataset identifiers** → Specific dataset versions ([[FlowSlices|mesh-resource.component.slice]]) 
* **Handle identifiers** → mesh knops themselves, see [[mesh-resource.component.knop-handle]]
...
Even though you might be tempted to think of datasets as concrete things, the IRIs for payload knops, flows, and versions all refer to concepts, i.e., **non-retrievable entities**. Only Distribution IRIs refer to downloadable data, i.e., dataset distributions.
